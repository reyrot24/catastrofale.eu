...
const Ubicazioni = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formLocale.section2.numeroPianiAttività) {
  ...
  return (
    <section className="w-full items-center bg-bg px-[5%] rounded-xl">
      <Toaster />
      <div className="flex flex-row  items-center gap-4 mb-4">
        <Button onClick={handleAddUbicazione} className="bg-brand-color">
          <Plus />
        </Button>
      ...
      <div className="flex flex-col gap-16 mb-8">
        {formState.ubicazioni.map((ubicazione) => (
          <div
            className="flex flex-row gap-16 items-center rounded p-4"
            key={ubicazione.id}
          >
            ...
            <div className="flex gap-4">
              <Button
                onClick={() => handleEditUbicazione(ubicazione)}
